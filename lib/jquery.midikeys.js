        ...
        var plugin = this,
            ...
            createEventCallback = function (status, velocity) {
                return function (event) {
                    ...
                    $el.trigger('message', createMIDIEvent(event, dataOptions));
                };
            };

        plugin.el = element;
        // save references of callback to support detachment if necessary
        plugin._keydown = createEventCallback(NOTE_ON, opts[PARAM_NOTE_ON_VELOCITY]);
        plugin._keyup = createEventCallback(NOTE_OFF, opts[PARAM_NOTE_OFF_VELOCITY]);
